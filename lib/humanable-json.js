...
// load built-in transforms (for browserify to resolve requires)
...
function parseJSON(obj, opts) {
  ...
  var excludedKeys = opts.excludedKeys || [];
  ...
  function startCollection(collectionType, meta) {
    if (excludedKeys.indexOf(meta.key) > -1) { return; }
...
// export humanable to window global
if (typeof window !== 'undefined') { window.humanable = humanable; }

// export humanable as angular module if angular is found
if ((typeof window !== 'undefined' && typeof window.angular !== 'undefined') || typeof angular !== 'undefined') {
  var ng = angular || window.angular;
  ng.module('humanable', [])
    .directive('humanable', function () {
      return {
        restrict: 'E',
        scope: {
          json: '=json',
        ...
        link: function (scope, element) {
          scope.$watchGroup(['json', 'opts'], function (newValues) {
            element.html(humanable.parseJSON(newValues[0], newValues[0]));
          });
        }
      };
    });
...
// export humanable for nodejs / browserify
